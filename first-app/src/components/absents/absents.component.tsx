...
const Absents = (props: IProps) => {
  ...
  const resetAbsent = () => {
    ...
    if (dispatch) {
      dispatch({
        type: "UPDATE_ABSENTS",
        payload: { id: props.id, change: -absents },
      });
    }
  };
  ...
  return (
    <div className="absents">
      ...
      <button disabled={!user} onClick={addAbsent}>
        +
      </button>
      <button disabled={!user} onClick={removeAbsent}>
        -
      </button>
      <button disabled={!user} onClick={resetAbsent}>
        Reset
      </button>
    </div>
  );
